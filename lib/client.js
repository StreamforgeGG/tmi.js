...
client.prototype._openConnection = function _openConnection() {
    if (this.opts.useProxy === true) {
		const selectedProxy = this.opts.proxies[Math.floor(Math.random() * this.opts.proxies.length)]
		const [host, port, username, password] = selectedProxy.split(':')

		let x = `http://${username}:${password}@${host}:${port}`

		// console.log(`Using proxy, selected:`, x)
        let y = new HttpsProxyAgent(x);
        if(this.opts.proxies.length > 0) this.ws = new ws(`${this.secure ? "wss" : "ws"}://${this.server}:${this.port}/`, { agent: y });
        else this.ws = new ws(`${this.secure ? "wss" : "ws"}://${this.server}:${this.port}/`, "irc");
